...
def get_employee_details(name: str) -> Dict[str, str]:
    """
    Get employee details by exact employee name.
    """
    ...
    if len(matches) > 1:
        raise ValueError(
            f"Multiple employees found with name '{name}'. Please use employee ID."
        )

    return employee_manager.get_employee_details(matches[0])
